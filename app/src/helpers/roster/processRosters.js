const processRosters = (rosters, players, owners) => {
  if (rosters && players && owners && owners.length > 1 && players.length > 1 ) {
    ...
    return {
      ...
      wrRank: uploadPlayersToRosters.sort((a, b) => parseFloat(b.kct.wr.total) - parseFloat(a.kct.wr.total)).map((roster, idx) => { return { rank: idx + 1, kct: roster.kct } }),
      teRank: uploadPlayersToRosters.sort((a, b) => parseFloat(b.kct.te.total) - parseFloat(a.kct.te.total)).map((roster, idx) => { return { rank: idx + 1, kct: roster.kct } }),
    }
  } else return []
}
